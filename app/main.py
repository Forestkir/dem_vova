...
class DataService:
    ...
    def parse_article_pairs(text: str):
        # Ожидаемый формат: "Артикул1, Кол-во1, Артикул2, Кол-во2"
...
class LoginDialog(QDialog):
    def __init__(self):
        ...
        # Визуальные подсказки (placeholders) для пользователя
    ...
    def login_click(self):
        ...
        if not login or not password:
            # Предупреждающее информативное сообщение (по ТЗ)
            show_warn(self, "Поля логина и пароля не могут быть пустыми.\nПожалуйста, заполните их.")
            return
        ...
        if user is None:
            show_error(self, "Пользователь не найден или пароль неверен.\nПроверьте данные и попробуйте снова.")
            return
...
class ProductFormDialog(QDialog):
    ...
    def resolve_photo_path(self, photo_file: str):
        if photo_file:
            ...
            # Сначала ищем рядом с .exe (новые фото)
            p = os.path.join(PHOTOS_DIR, file_name)
            if os.path.exists(p):
                return p

            # Затем ищем во вшитых стартовых фотках
            p_temp = os.path.join(PHOTOS_DIR_TEMP, file_name)
    ...
    def choose_photo(self):
        ...
        pix = QPixmap(path)
        if pix.isNull():
            show_warn(self, "Не удалось прочитать изображение. Попробуйте другой файл.")
            return
        if pix.width() > 300 or pix.height() > 200:
            show_warn(self, "Размер фото не должен превышать 300x200 пикселей (см. ТЗ).")
            return
    ...
    def save_click(self):
        ...
        try:
            ...
            show_error(self, "Ошибка ввода чисел. Проверьте формат цены, количества и скидки (используйте цифры).")
            return
...
class OrderFormDialog(QDialog):
    ...
    def save_click(self):
        ...
        if not articles:
            show_warn(self, "Поле 'Артикул' обязательно для заполнения.")
            return

        try:
            DataService.parse_article_pairs(articles)
        except Exception as ex:
            show_error(self, f"Ошибка в формате артикулов:\n{str(ex)}")
            return
...
class OrdersDialog(QDialog):
    ...
    def edit_order(self):
        ...
        if not order_id:
            show_warn(self, "Пожалуйста, сначала выберите заказ для редактирования в таблице.")
            return
    ...
    def delete_order(self):
        ...
        if not order_id:
            ...
            return
        ans = QMessageBox.question(self, "Подтверждение", "Удалить выбранный заказ? Эта операция необратима.")
        if ans != QMessageBox.StandardButton.Yes:
            return
        try:
            DataService.delete_order(order_id)
        except Exception as ex:
            show_error(self, f"Ошибка при удалении:\n{ex}")
            return
        self.load_orders()
...
class ProductsWindow(QMainWindow):
    def __init__(self, user_data: UserInfo):
        ...
        # Блок фильтрации, поиска и сортировки
        ...
        # Добавлена сортировка по цене согласно ТЗ
        ...
        # Сигналы реального времени для фильтров и поиска
    ...
    def load_suppliers(self):
        # Блокировка сигналов при заполнении ComboBox, чтобы избежать ложных срабатываний фильтра
    ...
    def photo_pixmap(self, photo_file: str):
        if photo_file:
            ...
            # Сначала ищем рядом с .exe (новые загруженные фото)
            p = os.path.join(PHOTOS_DIR, file_name)
            if os.path.exists(p):
                pix = QPixmap(p)
                if not pix.isNull():
        ...
        # Если фото нигде нет, возвращаем вшитую заглушку
        pix = QPixmap(PICTURE_PNG)
        return pix

        # Если нет даже заглушки - создаем серый квадрат (защита от вылета C++)
        fallback = QPixmap(90, 60)
        fallback.fill(QColor(Qt.GlobalColor.lightGray))
        return fallback

    def apply_filters(self):
        # Совмещение поиска, фильтрации и сортировки в реальном времени
        ...
        if self.is_manager_or_admin():
            ...
            # 1. Сначала фильтр по поставщику
            ...
            # 3. Затем сортировка оставшихся данных
    ...
    def fill_table(self, rows):
        ...
        for i, row in enumerate(rows):
            ...
            # Проверяем, удалось ли загрузить картинку. Если pix.isNull(),
            # вызов pix.scaled(...) мгновенно обрушит программу с кодом 0xC0000409
            if not pix.isNull():
                try:
                    img_label.setPixmap(pix.scaled(90, 60, Qt.AspectRatioMode.KeepAspectRatio,
                                                   Qt.TransformationMode.SmoothTransformation))
                except Exception:
    ...
    def add_product(self):
        ...
        if self.product_editor_opened:
            show_warn(self, "Окно добавления товара уже открыто.")
            return
    ...
    def edit_product(self):
        ...
        if not product_id:
            show_warn(self, "Пожалуйста, сначала выберите товар в таблице для редактирования.")
            return
        if self.product_editor_opened:
    ...
    def delete_product(self):
        ...
        product_id = self.selected_product_id()
        if not product_id:
            show_warn(self, "Выберите товар для удаления, кликнув на него в таблице.")
            return
        if DataService.product_exists_in_orders(product_id):
            show_warn(self, "Этот товар сейчас присутствует в заказах. Его удаление невозможно для сохранения истории.")
            return
        ans = QMessageBox.question(self, "Подтверждение", "Вы уверены, что хотите безвозвратно удалить этот товар?")
        if ans != QMessageBox.StandardButton.Yes:
            return
        try:
            DataService.delete_product(product_id)
        except Exception as ex:
            show_error(self, f"Критическая ошибка при удалении товара:\n{ex}")
            return
        self.load_suppliers()
        self.load_products()
...
def main():
    ...
    # Применение белой темы
    app.setStyleSheet(WHITE_THEME_QSS)
